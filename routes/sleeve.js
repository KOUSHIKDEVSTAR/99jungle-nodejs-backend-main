...
const QUERY = require('../config/db-query/sleeve');
...
router.post('/getall', async function(req, res, next) {
    try {
        ...
        if(qr && qr.length <= 0){
            ...
            //TAG not exist
            RESPONCE.successResponce(res, RESPONSE_MESSAGES.tagNotFound, [], null, ITEM);
        }else if(qr && qr.length > 0){
...
//-------------------------------------------------------------
//  get SLEEVES by filter data API
//-------------------------------------------------------------
router.post('/getbyfilter', async function(req, res, next) {
    try {
        ...
        if(qr && qr.length <= 0){
            //TAG not exist
            RESPONCE.errorResponce(res, RESPONSE_MESSAGES.tagNotFound, [], null, ITEM);
        }else if(qr && qr.length > 0){
            // TAG exist 
            RESPONCE.successResponce(res, RESPONSE_MESSAGES.tagFound, qr, null, ITEM);
        }
